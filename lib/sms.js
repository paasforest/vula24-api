...
async function sendLaunchApprovalSms(locksmithPhone, customerCode, tier) {
  ...
  const message =
    `Welcome to Vula24!\n` +
    `Your application is approved (${tier}). You are a founding member — no platform subscription fee for ${months} months once your account is activated.\n` +
    `Your code: ${customerCode}\n` +
    `Open your portal:\n${portalUrl}\n` +
    `If we asked for verification or proof, use:\n${paymentUrl}\n` +
    `Paid plans planned from ${billing} (we will give notice before any charge).`;
...
async function sendLaunchActivateSms(locksmithPhone, customerCode) {
  ...
  const billing = firstBillingMonthLabel();
  const msg =
    `Vula24: Your account is active — job leads by SMS.\n` +
    `Founding period: ${months} months with no platform fee from today.\n` +
    `Code: ${customerCode}\n` +
    `Paid plans from ${billing} — we will give notice.`;
