...
async function sendJobAlert(locksmithPhone, job) {
  ...
  const msg =
    ...
    `Urgency: ${urgencyEmoji} ${urgencyLabel}\n` +
    `Area: ${job.suburb}\n` +
    addressLine +
    customerName +
    `Phone: ${job.customer_phone}\n` +
    `Reply CLAIM#${jc} to take this job.\n` +
    `First reply wins.`;
...
async function sendJobClaimed(locksmithPhone, customer) {
  ...
  const suburb = customer.suburb
    ? `Area: ${customer.suburb}\n`
    : "";

  const msg =
    `Vula24: Job ${jc} is yours!\n` +
    `Customer: ${customer.name || "Customer"}\n` +
    `Phone: ${customer.phone}\n` +
    suburb +
    addressLine +
    `Call them now to confirm the job.`;
...
async function sendNoLocksmithAlert(adminPhone, job) {
  ...
  const addressLine = job.address
    ...
    : "";
  const msg =
    `Vula24 ALERT: No locksmith found!\n` +
    `Job: ${jc}\n` +
    `Service: ${job.service}\n` +
    `Area: ${job.suburb}${addressLine}\n` +
